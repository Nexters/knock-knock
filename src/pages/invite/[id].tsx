...
export default function Invite() {
  ...
  return (
    <SEO title="모임에 초대해요!" url={`/invite/${router.query.id}`}>
            ...
            <div className="mt-7 px-5">
              ...
              <div className="flex items-center flex-wrap">
                {eventData?.participates?.map(participate => {
                  return (
                    <div key={participate.id} className="badge badge-lg text-white mr-2 mb-2 text-sm max-w-[100px]">
                      <img className="w-[20px] h-[20px] rounded-full mr-2" src={participate.profile.image || ''} />
                      <span className="inline-block text-sm w-[90px] h-[20px] text-ellipsis overflow-hidden">
